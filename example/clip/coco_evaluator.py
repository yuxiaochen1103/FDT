...
class ClsSolver(BaseSolver):
    ...
    def setup_env(self):
        ...
        # create logger
        self.path.log_path = os.path.join(self.path.root_path, 'val_log.txt')
        ...
        self.state = torch.load(ckpt_pth, map_location='cpu')  # can load ckpt
    ...
    def evaluate(self, val_data):
        ...
        while begin_idx < cap_num:
            cap_emb = self.model.module.encode_text(all_cap[begin_idx: begin_idx + bz])
        ...
        for batch_idx, batch in enumerate(val_data['loader']):
            ...
            assert logits.shape[0] == images.shape[0] * self.dist.world_size == indexs.shape[0]
