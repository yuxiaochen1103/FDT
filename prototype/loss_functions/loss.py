...
def orthgonal_loss_fn(ft):
    ...
    #l2 norm
    ft = ft / (ft.norm(dim=-1, keepdim=True) + 1e-10)
    #cossim
    sim = ft @ ft.t() #cal cosine sim

    #calculate loss
    identity = torch.eye(n, device=ft.device)

    loss = ((sim - identity) ** 2).sum() / (n ** 2)
