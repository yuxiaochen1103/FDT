...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_dense=False, return_raw_feature=False, return_att=False):
        ...
        att = att[:, 0] #get cls token att

        #-----------project the features to common space
        if self.proj is not None:
            x = x @ self.proj

        ret = [x] #projected features
        if return_dense: #return raw features of tokens
            ret.append(dense_feat)
        if return_raw_feature: #return raw features
            ret.append(feature)
        if return_att:
            ret.append(att)
        if len(ret) == 1:
...
def visual_transformer_L14(**kwargs):
    vision_layers = 24
    vision_width = 1024
    vision_heads = vision_width // 64
...
def visual_transformer_L16(**kwargs):
    vision_layers = 24
    vision_width = 1024
    vision_heads = vision_width // 64
