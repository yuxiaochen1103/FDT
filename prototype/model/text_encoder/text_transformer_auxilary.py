...
from .base_transformer_auxilary import Transformer, LayerNorm
...
class TextTransformer(nn.Module):
    ...
    def forward(self, text, mask_type=None, return_dense=False, return_raw_feature=False, return_padmask=False, return_att=False):
        ...
        if self.text_encode_type == 'Transformer':
            ...
            #att = att[torch.arange(att.shape[0]), texts.argmax(dim=-1)]
            ...
            x = x[torch.arange(x.shape[0]), texts.argmax(dim=-1)] #raw feature
            if return_raw_feature:
                feat = x #feature without linear projection
                #print('raw txt ft shape:', feat.shape)

            x = self.text_projection(x) #get cls tokens features
            #print('projected txt ft shape:', x.shape)
        ...
        if mask_type is not None:
            return x, words_feat, labels, pad_mask

        ret = [x] #project feature
        if return_dense:
            ret.append(words_feat) #add word feature
        if return_raw_feature:
            ret.append(feat) #add raw features
...
def text_transformers_L(**kwargs):
    default_kwargs = {
        # 'embed_dim':1024, embed_dim from config
        'context_length':77,
        'transformer_layers':12,
